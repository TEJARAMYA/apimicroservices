...
export default function(app: Express, impl: t.UserApi) {
	...
	app.delete(
		...
		function (req, res) {
			try {
				impl.deleteUser(v.parseString('params.Topicid', req.params['Topicid'])).then(function (response) {
					if (response.status === 201) {
						let body: any
						try {
							body = v.modelApiApiResponseToJson('response', response.body)
	...
	app.put(
		...
		function (req, res) {
			try {
				...
				impl.updateUser(v.parseString('params.Topicid', req.params['Topicid']), __body()).then(function (response) {
					if (response.status === 201) {
						let body: any
						try {
							body = v.modelApiApiResponseToJson('response', response.body)
